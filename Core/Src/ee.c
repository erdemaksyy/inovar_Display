...
bool ee_format(bool keepRamData)
{
  ...
  flashErase.Page = (((uint32_t)0x08000800) | (_EE_SIZE * _EE_USE_FLASH_PAGE_OR_SECTOR));
...
bool ee_write(uint32_t startVirtualAddress, uint32_t len, uint8_t *data)
{
  ...
  {		
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, ((i + startVirtualAddress)) + (((uint32_t)0x08000800) | (_EE_SIZE * _EE_USE_FLASH_PAGE_OR_SECTOR)), (uint64_t)(data[i])) != HAL_OK)
    {
      HAL_FLASH_Lock();
      return false;
    }
  ...
  {		
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, ((i + startVirtualAddress)) + (((uint32_t)0x08000800) | (_EE_SIZE * _EE_USE_FLASH_PAGE_OR_SECTOR)), (uint64_t)(data[i] | (data[i+1] << 8))) != HAL_OK)
    {
      HAL_FLASH_Lock();
      return false;
    }
  ...
  for (uint32_t i = 0; i < len; i += 8)
  {
    ...
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, ((i + startVirtualAddress)) + (((uint32_t)0x08000800) | (_EE_SIZE * _EE_USE_FLASH_PAGE_OR_SECTOR)), data64) != HAL_OK)
    {
      HAL_FLASH_Lock();
      return false;
    }
